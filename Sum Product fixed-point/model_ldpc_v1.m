...
indexLine=find(A);
nzero=nnz(A);
% noisevariance=[0.7079];%[ 0.3162 0.3548 0.3981 0.4467 0.5012 ];%0.6166  0.6310 0.6607   0.6761  0.6918 0.7079 0.7943 1.0];%[0.001 0.1000 0.1259 0.1585 0.1995 0.2512 0.3162 0.3981 0.5012]; %[0.7079];%[0.6607 0.4467   0.3981    0.3162    0.2512    0.1995  0.1259];
...
% noisevariance = [0.609536897240169,0.616595001861482,0.623734835482419,0.630957344480193,0.638263486190549,0.645654229034656,0.653130552647472,0.660693448007596,0.668343917568615,0.676082975391982,0.683911647281429,0.691830970918937,0.707945784384138, 0.73, 0.76, 0.794328234724282, 0.8, 0.9, 1];%[0.6237    0.6310    0.6383    0.6457    0.6531    0.6607    0.6683];%[0.5957    0.6026    0.6095    0.6166    0.6237];%[0.6310    0.6383    0.6457    0.6531    0.6607    0.6683    0.6761    0.6839    0.6918];
...
%        [receivedBits, totalIter] = Belief_propagation_lambda_min(A, decoderInput, L, noisevariance(i), codeRate, Eb, inCheck, inBit, indexLine);
%        [receivedBits, totalIter] = Belief_propagation_lambda_min_ofset(A, decoderInput, L, noisevariance(i), codeRate, Eb, inCheck, inBit, indexLine);
%        [receivedBits, totalIter] = Belief_propagation_lambda_min_ofset_fixedpoint(A, decoderInput, L, noisevariance(i), codeRate, Eb, inCheck, inBit, indexLine);
