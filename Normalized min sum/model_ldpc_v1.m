...
indexLine=find(H);
nzero=nnz(H);
noisevariance=[0.7079];                                                                                     %Noise input 
%  noisevariance=[0.001 0.1000 0.1259 0.1585 0.1995 0.2512 0.3162 0.3981 0.5012 0.6310 0.7943 1.0000];
% noisevariance=[0.6166  0.6310 0.6607   0.6761  0.6918 0.7079 0.7943 0.8318 0.8913 1.0];
rounds=size(noisevariance,2);
max=zeros(1,rounds);
Eb2No=zeros(1,rounds);
L=50;                                                                         %max iterations for decoder
frames=500;                                                                   %max frames if no errors (or close to)                           
...
frameError=0;
maxIter = zeros(1, rounds);
minIter = zeros(1, rounds);
AverageRoundIter = zeros(1, rounds);
...
%Lc=2*sqrt(codeRate*Eb)/noisevariance(i);
    ...
    %for counter = 1:frames
     while errors<300                                                      %rule of thumb, stop after 100 errors detected
...
%         decoderInput = demodSignal;
        [receivedBits, totalIter] = Normalized_min_sum(A, decoderInput, L, noisevariance(i), inBit, indexLine);  %my gallagher decoder
...
%         [receivedBits, totalIter] = ldpc_min_sum_v3(A, decoderInput, noisevariance(i), L, inBit);
        ...
        if totalIter >maxIter(i)
            maxIter(i) = totalIter;
        end
        if totalIter < minIter(i)
            minIter(i) = totalIter;
        end
        IterationTotal = totalIter + IterationTotal;
     end
    AverageRoundIter(i) = IterationTotal/counter(i);
